...
const SortableItem = sortableElement(({index, title, artist}) => {
	return (
		<ListItem divider style={{maxWidth: '500px' }}>
			<ListItemText>
...
class Playlist extends Component {
	...
	get playlistTrackId() {
		...
		return playlist.length ? (parseInt(playlist[playlist.length - 1].id) + 1).toString() : '1';
	}
			...
			<div className="playlist">
				<SortableContainer onSortEnd={this.onSortEnd}>
					{playlist.map((track, index) => (
						<SortableItem key={`item-${index}`} index={index} index={index} title={track.title} artist={track.artist} />
					))}
				</SortableContainer>
			</div>
		)
